...
name = "audit-log"
tool_match = ["Bash"]
command = "echo \"$(date): ${TOOL_INPUT_COMMAND}\" >> .eggsplain/audit.log"
...
```bash
# CLI — logs to /tmp/eggsplain-logs at debug level
eggsplain --log-dir /tmp/eggsplain-logs --log-level debug "Read Cargo.toml"
```
...
| `"eggsplain_providers=debug,info"` | Debug for providers, info for everything else |
...
| Platform | Path |
|----------|------|
| macOS | `~/Library/Logs/eggsplain/` |
| Linux | `$XDG_STATE_HOME/eggsplain/logs/` or `~/.local/state/eggsplain/logs/` |
| Windows | `{data_local_dir}/eggsplain/logs/` |
...
```json
{"timestamp":"2026-05-13T12:12:52.431Z","level":"INFO","fields":{"message":"mcp server connected","server":"sentry","tools":20},"target":"eggsplain_mcp","spans":[{"name":"agent_run","session_id":"abc-123","msg_id":"msg-456"}]}
```
...
| Field | Description |
|-------|-------------|
| `target` | Source crate (`eggsplain_agent`, `eggsplain_providers`, `eggsplain_mcp`, etc.) |
...
grep '"session_id":"abc-123"' 2026-05-13.eggsplain.log | jq .
```

### Library Integration

When eggsplain is used as a library (e.g. embedded in a backend server), the `create_file_layer()` API provides a composable tracing layer:

```rust
use eggsplain_config::logging::{ResolvedLogging, create_file_layer};

let resolved = ResolvedLogging {
    enabled: true,
    level: "eggsplain_agent=debug,eggsplain_providers=debug".to_string(),
...
tracing_subscriber::registry()
    .with(your_app_layer)
    .with(layer)  // eggsplain logs → separate eggsplain.log file
    .init();
```

The host application owns the global subscriber; eggsplain library crates only emit tracing events and never initialize a subscriber themselves.
...
1. **Global**: `<config_dir>/eggsplain/AGENTS.md` — user-level instructions for all projects
...
Running eggsplain in `packages/server/` produces a system prompt containing both files, workspace first, then server.
...
```
<config_dir>/eggsplain/projects/<sanitized-project-path>/memory/
...
```bash
export EGGSPLAIN_MEMORY_DIR=/custom/path
```
...
plan_directory = ".eggsplain/plans"  # Where plan files are saved
```
