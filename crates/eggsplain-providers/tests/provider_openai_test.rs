...
use eggsplain_providers::openai::OpenAIProvider;
use eggsplain_types::llm::{LlmEvent, LlmRequest};
use eggsplain_types::message::{ContentBlock, Message, Role, StopReason};
use serde_json::json;
...
async fn test_openai_api_error_non_success_status() {
    ...
    match result.unwrap_err() {
        eggsplain_providers::ProviderError::Api { status, .. } => {
...
async fn test_openai_rate_limited() {
    ...
    assert!(result.is_err());
    match result.unwrap_err() {
        eggsplain_providers::ProviderError::RateLimited { retry_after_ms } => {
