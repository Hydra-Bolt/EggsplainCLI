...
use eggsplain_types::llm::{LlmEvent, LlmRequest, ThinkingConfig};
use eggsplain_types::message::{StopReason, TokenUsage};

use super::anthropic_shared;
use crate::{LlmProvider, ProviderError};
use eggsplain_config::compat::{self, ProviderCompat};
...
impl LlmProvider for BedrockProvider {
    async fn stream(
        ...
        tracing::debug!(target: "eggsplain_providers", body = %serde_json::to_string_pretty(&body).unwrap_or_default(), "outgoing request");
        ...
        while let Some((event_data, consumed)) = parse_aws_event(&buffer) {
            ...
            if let Some(payload) = event_data {
                ...
                if let Ok(wrapper) = serde_json::from_slice::<Value>(&payload) {
                    ...
                    {
                        tracing::debug!(target: "eggsplain_providers", chunk = %inner, "bedrock event chunk");
...
/// Build AwsCredentials from eggsplain-config's BedrockConfig
pub fn credentials_from_config(bc: &eggsplain_config::config::BedrockConfig) -> AwsCredentials {
