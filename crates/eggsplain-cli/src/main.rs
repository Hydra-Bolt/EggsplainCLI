...
use eggsplain_agent::bootstrap::AgentBootstrap;
use eggsplain_agent::output::OutputSink;
use eggsplain_agent::output::protocol_sink::ProtocolSink;
use eggsplain_agent::output::terminal::TerminalSink;
use eggsplain_agent::session;
use eggsplain_config::auth;
use eggsplain_config::config::{self, CliArgs, Config, McpServerConfig, TransportType};
use eggsplain_mcp::manager::McpManager;
use eggsplain_mcp::tool_proxy::register_single_server_tools;
use eggsplain_protocol::commands::{ApprovalScope, ProtocolCommand};
use eggsplain_protocol::events::ProtocolEvent;
use eggsplain_protocol::reader::spawn_stdin_reader;
use eggsplain_protocol::writer::{ProtocolEmitter, ProtocolWriter};
use eggsplain_protocol::{ToolApprovalManager, ToolApprovalResult};
...
struct Cli {
    ...
    /// Project directory to load .eggsplain.toml from (defaults to CWD)
    #[arg(long)]
    ...
    /// Log level filter (e.g. "info", "debug", "info,eggsplain_providers=debug")
    #[arg(long)]
...
async fn main() -> anyhow::Result<()> {
    ...
    if cli.login || cli.logout {
        ...
        if cli.login {
            oauth.login().await?;
            eprintln!("Login successful! You can now use eggsplain without --api-key.");
        } else {
            oauth.logout()?;
        }
        return Ok(());
    ...
    let _log_guard = {
        ...
        if resolved.enabled {
            match eggsplain_config::logging::create_file_layer(&resolved) {
                Ok((layer, guard)) => {
...
async fn repl_loop(
    engine: &mut eggsplain_agent::engine::AgentEngine,
    ...
    loop {
        ...
        match engine.run(input, "").await {
            ...
            Err(eggsplain_agent::engine::AgentError::UserAborted) => break,
            Err(e) => {
    ...
    while let Some(cmd) = cmd_rx.recv().await {
        match cmd {
            ProtocolCommand::AddMcpServer {
                ...
                tracing::info!(target: "eggsplain_mcp", %name, %transport, ?command, "AddMcpServer received");
                let config =
                ...
                tracing::info!(target: "eggsplain_mcp", %name, "connecting to mcp server");
                match McpManager::connect_all(&single_configs).await {
                    Ok(mgr) => {
                        let tool_names: Vec<String> = mgr
                            ...
                            .collect();
                        tracing::info!(target: "eggsplain_mcp", %name, tools = tool_names.len(), "mcp server connected");
                    ...
                    Err(e) => {
                        tracing::warn!(target: "eggsplain_mcp", %name, error = %e, "mcp server connection failed");
    ...
    loop {
        ...
        match cmd {
                            ...
                            Some(sub_cmd) = cmd_rx.recv() => {
                                match sub_cmd {
                                    ...
                                    ProtocolCommand::SetConfig { model, thinking, thinking_budget, effort, compaction } => {
                                        ...
                                        let _ = writer.emit(&eggsplain_protocol::events::ProtocolEvent::Info {
                                            msg_id: String::new(),
                                    ...
                                    ProtocolCommand::Ping => {
                                        let _ = writer.emit(&eggsplain_protocol::events::ProtocolEvent::Pong);
                                    }
                                    _ => {
                                        tracing::debug!(target: "eggsplain_protocol", "ignoring command during active message processing");
                                    }
                                }
                            }
                        }
            ...
            ProtocolCommand::InitHistory { text } => {
                tracing::debug!(target: "eggsplain_protocol", chars = text.len(), "InitHistory received");
            }
            ProtocolCommand::SetMode { mode } => {
                ...
                tracing::debug!(target: "eggsplain_protocol", mode = %mode_str, "SetMode applied");
            }
