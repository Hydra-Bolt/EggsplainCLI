    ...
    pub async fn connect_all(configs: &HashMap<String, McpServerConfig>) -> Result<Self, McpError> {
        ...
        for (name, config) in configs {
            match Self::connect_server(name, config).await {
                Ok(server) => {
                    tracing::info!(target: "eggsplain_mcp", server = %name, tools = server.tools.len(), resources = server.supports_resources, "mcp server connected");
                    servers.insert(name.clone(), server);
                }
                Err(e) => {
                    // Non-fatal: continue with other servers
                    tracing::warn!(target: "eggsplain_mcp", server = %name, error = %e, "mcp server connection failed");
                }
            }
        }
    ...
    pub async fn shutdown(&self) {
        for (name, server) in &self.servers {
            if let Err(e) = server.transport.close().await {
                tracing::warn!(target: "eggsplain_mcp", server = %name, error = %e, "error closing mcp server");
            }
        }
    }
