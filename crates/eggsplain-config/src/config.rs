...
use eggsplain_types::llm::ThinkingConfig;
...
fn resolve_api_key(
    ...
    anyhow::bail!(
        "No API key found. Provide via --api-key, config file, environment variable \
         (API_KEY, ANTHROPIC_API_KEY, or OPENAI_API_KEY), or run 'eggsplain --login'."
    )
}
...
///
/// - Linux:   `~/.config/eggsplain`
/// - macOS:   `~/Library/Application Support/eggsplain`
/// - Windows: `%APPDATA%\eggsplain`
pub fn app_config_dir() -> Option<PathBuf> {
    dirs::config_dir().map(|d| d.join("eggsplain"))
}
...
pub fn global_config_path() -> PathBuf {
    app_config_dir()
        .unwrap_or_else(|| PathBuf::from("eggsplain"))
        .join("config.toml")
}

fn project_config_path() -> PathBuf {
    PathBuf::from(".eggsplain.toml")
}

fn load_config_file(path: &Path) -> ConfigFile {
    match std::fs::read_to_string(path) {
        Ok(content) => toml::from_str(&content).unwrap_or_else(|e| {
            tracing::warn!(target: "eggsplain_config", path = %path.display(), error = %e, "failed to parse config file");
            ConfigFile::default()
        }),
...
pub fn init_config() -> anyhow::Result<()> {
    ...
    if path.exists() {
        tracing::info!(target: "eggsplain_config", path = %path.display(), "config file already exists");
        return Ok(());
    }
    ...
    tracing::info!(target: "eggsplain_config", path = %path.display(), "config file created");
    Ok(())
}

const DEFAULT_CONFIG_TEMPLATE: &str = r#"# eggsplain configuration
...
[session]
enabled = true
directory = ".eggsplain/sessions"  # relative to project root
...
# dir = "~/Library/Logs/eggsplain"    # log directory (default: platform-specific)
...
"#;
    ...
    fn test_resolve_with_project_dir_loads_project_config() {
        ...
        let project_toml = tmp.path().join(".eggsplain.toml");
        std::fs::write(
            &project_toml,
            r#"
[default]
