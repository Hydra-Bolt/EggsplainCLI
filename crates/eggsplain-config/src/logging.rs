...
pub fn default_log_dir() -> PathBuf {
    ...
    {
        dirs::home_dir()
            .map(|h| h.join("Library").join("Logs").join("eggsplain"))
            .unwrap_or_else(|| PathBuf::from("eggsplain/logs"))
    }
    #[cfg(target_os = "linux")]
    {
        dirs::state_dir()
            .map(|d| d.join("eggsplain").join("logs"))
            .unwrap_or_else(|| {
                dirs::home_dir()
                    .map(|h| h.join(".local").join("state").join("eggsplain").join("logs"))
                    .unwrap_or_else(|| PathBuf::from("eggsplain/logs"))
            })
    }
...
"#;
    ...
    #[test]
    fn default_log_dir_contains_eggsplain() {
        let dir = default_log_dir();
