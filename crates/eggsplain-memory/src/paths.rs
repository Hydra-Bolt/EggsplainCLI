...
const MEMORY_DIR_ENV: &str = "EGGSPLAIN_MEMORY_DIR";
...
///
/// Resolution order:
///   1. `EGGSPLAIN_MEMORY_DIR` environment variable (explicit override)
///   2. `app_config_dir()` from `eggsplain-config` (platform-aware default)
///
...
pub fn memory_base_dir() -> Option<PathBuf> {
    ...
    eggsplain_config::config::app_config_dir()
}
