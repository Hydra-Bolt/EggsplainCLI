...
async fn load_skill_file(
    ...
    // used for ${EGGSPLAIN_SKILL_DIR} variable substitution in skill content.
...
///
/// Examples:
/// - base=`<config_dir>/eggsplain/skills`, target=`<config_dir>/eggsplain/skills/db/migrate` → `"db:migrate"`
/// - base=`<config_dir>/eggsplain/skills`, target=`<config_dir>/eggsplain/skills/my-skill` → `"my-skill"`
