...
/// 5. Skill directory: `${EGGSPLAIN_SKILL_DIR}` → `skill_root`
/// 6. Session ID: `${EGGSPLAIN_SESSION_ID}` → `session_id`
...
pub fn substitute_arguments(
    ...
    // 5. ${EGGSPLAIN_SKILL_DIR}
    if let Some(root) = skill_root {
        result = result.replace("${EGGSPLAIN_SKILL_DIR}", root);
    }

    // 6. ${EGGSPLAIN_SESSION_ID}
    if let Some(sid) = session_id {
        result = result.replace("${EGGSPLAIN_SESSION_ID}", sid);
    }
...
mod tests {
    ...
    #[test]
    fn test_eggsplain_skill_dir_substitution() {
        let result = substitute_arguments(
            "dir=${EGGSPLAIN_SKILL_DIR}",
            None,
            &[],
            Some("/my/skill"),
            None,
    ...
    #[test]
    fn test_eggsplain_session_id_substitution() {
        let result = substitute_arguments(
            "sid=${EGGSPLAIN_SESSION_ID}",
            None,
            &[],
            None,
            Some("sess-123"),
...
mod supplemental_tests {
    ...
    // -----------------------------------------------------------------------
    // TC-6.x: ${EGGSPLAIN_SKILL_DIR} substitution
    // -----------------------------------------------------------------------

    #[test]
    fn tc_6_1_skill_dir_replaced() {
        let r = substitute_arguments(
            "cd ${EGGSPLAIN_SKILL_DIR}",
            None,
            &[],
            Some("/home/user/.eggsplain/skills/my-skill"),
            None,
        );
        assert_eq!(r, "cd /home/user/.eggsplain/skills/my-skill");
    }

    #[test]
    fn tc_6_2_skill_dir_none_not_replaced() {
        // skill_root = None → ${EGGSPLAIN_SKILL_DIR} stays unreplaced
        let r = substitute_arguments("cd ${EGGSPLAIN_SKILL_DIR}", None, &[], None, None);
        assert_eq!(r, "cd ${EGGSPLAIN_SKILL_DIR}");
    }

    #[test]
    fn tc_6_3_skill_dir_multiple_occurrences() {
        let r = substitute_arguments(
            "${EGGSPLAIN_SKILL_DIR}/a and ${EGGSPLAIN_SKILL_DIR}/b",
            None,
            &[],
            Some("/skills/foo"),
            None,
    ...
    // -----------------------------------------------------------------------
    // TC-7.x: ${EGGSPLAIN_SESSION_ID} substitution
    // -----------------------------------------------------------------------

    #[test]
    fn tc_7_1_session_id_replaced() {
        let r = substitute_arguments(
            "Session: ${EGGSPLAIN_SESSION_ID}",
            None,
            &[],
            None,
            Some("abc-123"),
    ...
    fn tc_7_2_session_id_none_not_replaced() {
        let r = substitute_arguments("Session: ${EGGSPLAIN_SESSION_ID}", None, &[], None, None);
        assert_eq!(r, "Session: ${EGGSPLAIN_SESSION_ID}");
    }
    ...
    fn tc_9_1_multiple_placeholder_types() {
        let r = substitute_arguments(
            "cd ${EGGSPLAIN_SKILL_DIR} && run $ARGUMENTS[0] with $ARGUMENTS",
            Some("alpha beta"),
            &[],
            Some("/skills/foo"),
            None,
