...
use eggsplain_config::config::app_config_dir;

// ---------------------------------------------------------------------------
// User-level directories (<config_dir>/eggsplain/)
// ---------------------------------------------------------------------------

/// Return the user-level skills directory: `<config_dir>/eggsplain/skills/`
///
...
/// Return the user-level legacy commands directory: `<config_dir>/eggsplain/commands/`
...
/// Find all project-level `.eggsplain/skills/` directories by walking up from
...
/// Find all project-level `.eggsplain/commands/` directories (legacy), same walk.
...
///
/// Each path in `add_dirs` is checked for a `.eggsplain/skills/` subdirectory.
/// Only directories that exist are included.
pub fn additional_skills_dirs(add_dirs: &[PathBuf]) -> Vec<PathBuf> {
    add_dirs
        .iter()
        .map(|d| d.join(".eggsplain").join("skills"))
        .filter(|p| p.is_dir())
        .collect()
}
...
/// `.eggsplain/<subdir>/` directories that exist. Returns deepest-first.
fn walk_up_dirs(cwd: &Path, subdir: &str) -> Vec<PathBuf> {
    ...
    loop {
        let candidate = current.join(".eggsplain").join(subdir);
        if candidate.is_dir() {
            dirs.push(candidate);
        }
...
mod tests {
    ...
    #[test]
    fn test_user_skills_dir_contains_eggsplain_skills() {
        if let Some(dir) = user_skills_dir() {
            let s = dir.to_string_lossy();
            assert!(s.contains("eggsplain"), "expected 'eggsplain' in path: {s}");
            assert!(
                s.ends_with("skills"),
    ...
    #[test]
    fn test_user_commands_dir_contains_eggsplain_commands() {
        if let Some(dir) = user_commands_dir() {
            let s = dir.to_string_lossy();
            assert!(s.contains("eggsplain"));
    ...
    fn test_project_skills_dirs_finds_dirs() {
        ...
        make_dir(root, ".eggsplain/skills");
        let nested = root.join("sub").join("project");
        fs::create_dir_all(&nested).unwrap();
        make_dir(&nested, ".eggsplain/skills");
    ...
    fn test_additional_skills_dirs_existing() {
        let tmp = TempDir::new().unwrap();
        make_dir(tmp.path(), ".eggsplain/skills");
...
mod supplemental_tests {
    ...
    fn tc_2_1_user_skills_dir_ends_with_skills() {
        if let Some(dir) = user_skills_dir() {
            ...
            assert!(s.contains("eggsplain"), "path should contain 'eggsplain': {s}");
        }
    }
    ...
    fn tc_4_2_project_skills_dirs_nonexistent_subdir_not_returned() {
        let tmp = TempDir::new().unwrap();
        fs::create_dir(tmp.path().join(".git")).unwrap();
        // No .eggsplain/skills/ created
        let dirs = project_skills_dirs(tmp.path());
        assert!(
            dirs.is_empty(),
            "should be empty when .eggsplain/skills/ doesn't exist"
        );
    }
    ...
    fn tc_4_3_project_skills_dirs_deepest_first() {
        ...
        make_dir(&inner, ".eggsplain/skills");
    ...
    fn tc_4_4_project_skills_dirs_stops_at_git_root() {
        ...
        // .eggsplain/skills in grandparent (above git root) — should NOT be collected
        make_dir(grandparent, ".eggsplain/skills");

        let repo = grandparent.join("repo");
        fs::create_dir_all(&repo).unwrap();
        fs::create_dir(repo.join(".git")).unwrap();
        make_dir(&repo, ".eggsplain/skills");

        let sub = repo.join("sub");
        fs::create_dir_all(&sub).unwrap();

        let dirs = project_skills_dirs(&sub);
        // Only repo's .eggsplain/skills should be included
        assert!(
    ...
    fn tc_5_1_project_commands_dirs_finds_commands_dir() {
        ...
        let root = tmp.path();
        fs::create_dir(root.join(".git")).unwrap();
        make_dir(root, ".eggsplain/commands");

        let dirs = project_commands_dirs(root);
        assert_eq!(dirs.len(), 1);
        assert!(dirs[0].ends_with(".eggsplain/commands"));
    }
    ...
    fn tc_6_1_additional_skills_dirs_with_existing_subdir() {
        ...
        let result = additional_skills_dirs(&[tmp.path().to_path_buf()]);
        assert_eq!(result.len(), 1);
        assert!(result[0].ends_with(".eggsplain/skills"));
    }

    #[test]
    fn tc_6_2_additional_skills_dirs_no_subdir_skipped() {
        let tmp = TempDir::new().unwrap();
        // No .eggsplain/skills/ subdirectory
        let result = additional_skills_dirs(&[tmp.path().to_path_buf()]);
        assert!(result.is_empty());
    }
    ...
    fn tc_6_4_additional_skills_dirs_multiple_add_dirs() {
        ...
        make_dir(tmp1.path(), ".eggsplain/skills");
        make_dir(tmp2.path(), ".eggsplain/skills");
