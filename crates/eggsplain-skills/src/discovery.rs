...
/// Manages runtime discovery of `.eggsplain/skills/` directories found in
...
impl RuntimeDiscovery {
    ...
    /// Discover `.eggsplain/skills/` directories by walking up from each file path to `cwd`.
    ///
    ...
    pub async fn discover_dirs_for_paths(
        ...
        for &file_path in file_paths {
            ...
            loop {
                ...
                let skill_dir = current.join(".eggsplain").join("skills");
                ...
                if !self.checked_dirs.contains(&skill_dir) {
                    ...
                    if tokio::fs::metadata(&skill_dir).await.is_ok() {
                        ...
                        if is_path_gitignored(containing_dir, resolved_cwd).await {
                            tracing::debug!(target: "eggsplain_skills", path = %skill_dir.display(), "skipping gitignored skills directory");
                        } else {
    ...
    pub async fn add_skill_directories(&mut self, dirs: &[PathBuf]) -> usize {
        ...
        if added > 0 {
            tracing::info!(target: "eggsplain_skills", added, directories = dirs.len(), "dynamically discovered new skills");
        }
    ...
    /// already known not to contain a `.eggsplain/skills/` subdirectory.
    ...
    /// `.eggsplain/skills/` subdirectories.
    ///
