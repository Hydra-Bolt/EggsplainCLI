...
// AC-5: $ARGUMENTS, $0, ${EGGSPLAIN_SKILL_DIR} are correctly substituted
// ---------------------------------------------------------------------------
...
async fn tc_e2e_2_inline_variable_substitution() {
    ...
    assert!(
        ...
        "${{EGGSPLAIN_SKILL_DIR}} should be replaced with skill root, got: {result}"
    );
    ...
    assert!(
        !result.contains("${EGGSPLAIN_SKILL_DIR}"),
        "${{EGGSPLAIN_SKILL_DIR}} literal should not remain, got: {result}"
    );
}
...
// AC-14: legacy command files from .eggsplain/commands/ are loaded as SkillDefinition
// ---------------------------------------------------------------------------
...
async fn tc_e2e_11_legacy_commands_loaded() {
    ...
    // load_all_skills looks for .eggsplain/commands/ relative to cwd.
    let commands_dir = tmp.path().join(".eggsplain").join("commands");
    ...
    let skill = skills
        ...
        .expect("legacy-cmd should be loaded from flat .md file in .eggsplain/commands/");
