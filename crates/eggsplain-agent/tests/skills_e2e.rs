...
//! `.eggsplain/skills/` and `.eggsplain/commands/` layout, then exercises the full
...
use eggsplain_agent::context::{SystemPromptCache, build_system_prompt};
use eggsplain_agent::skill_tool::SkillTool;
use eggsplain_agent::skills::loader::load_all_skills;
use eggsplain_agent::skills::permissions::SkillPermissionChecker;
use eggsplain_agent::skills::types::SkillMetadata;
...
/// Create a project-like temp directory with `.git` marker and `.eggsplain/skills/` + `.eggsplain/commands/`.
...
fn make_project() -> (TempDir, PathBuf) {
    ...
    // Skills directory
    let skills_dir = root.join(".eggsplain").join("skills");
    fs::create_dir_all(&skills_dir).unwrap();

    // Commands directory (legacy)
    let commands_dir = root.join(".eggsplain").join("commands");
...
async fn e2_legacy_commands_discovered() {
    ...
    println!("E2 PASS: legacy command 'legacy-cmd' discovered from .eggsplain/commands/");
}
...
// ---------------------------------------------------------------------------
// E8: Full SkillTool execution (db:migrate with $ARGUMENTS + ${EGGSPLAIN_SKILL_DIR})
// ---------------------------------------------------------------------------
...
async fn e8_full_execution() {
    ...
    assert!(
        !result.content.contains("${EGGSPLAIN_SKILL_DIR}"),
        "E8 FAIL: ${{EGGSPLAIN_SKILL_DIR}} not expanded. Got: {}",
        result.content
    );
    println!("E8 PASS: full execution with $ARGUMENTS and ${{EGGSPLAIN_SKILL_DIR}} substitution");
}
