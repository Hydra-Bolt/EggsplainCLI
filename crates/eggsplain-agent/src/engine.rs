...
use eggsplain_config::compact::CompactConfig;
use eggsplain_config::config::Config;
use eggsplain_config::hooks::HookEngine;
use eggsplain_protocol::events::ToolCategory;
use eggsplain_providers::{LlmProvider, ProviderError, create_provider};
use eggsplain_tools::registry::ToolRegistry;
use eggsplain_types::llm::{LlmEvent, LlmRequest};
use eggsplain_types::message::{ContentBlock, Message, Role, StopReason, TokenUsage};
use eggsplain_types::skill_types::{ContextModifier, PlanModeTransition, effort_to_string};
...
pub struct AgentEngine {
    ...
    thinking: Option<eggsplain_types::llm::ThinkingConfig>,
    /// Resolved provider compat settings (for capability validation)
    compat: eggsplain_config::compat::ProviderCompat,
    ...
    approval_manager: Option<Arc<eggsplain_protocol::ToolApprovalManager>>,
    protocol_writer: Option<Arc<dyn eggsplain_protocol::writer::ProtocolEmitter>>,
    ...
    compaction_level: eggsplain_compact::CompactionLevel,
    toon_enabled: bool,
...
impl AgentEngine {
    ...
    pub fn compaction_level(&self) -> eggsplain_compact::CompactionLevel {
    ...
    pub fn compat(&self) -> &eggsplain_config::compat::ProviderCompat {
        &self.compat
    }
    ...
    pub fn init_session(
        ...
        if let Some(mgr) = &self.session_manager {
            ...
            tracing::info!(target: "eggsplain_agent", session_id = %session.id, provider = %provider_name, model = %self.model, "session started");
    ...
    pub fn set_approval_manager(&mut self, mgr: Arc<eggsplain_protocol::ToolApprovalManager>) {
        self.approval_manager = Some(mgr);
    }

    pub fn set_protocol_writer(&mut self, writer: Arc<dyn eggsplain_protocol::writer::ProtocolEmitter>) {
    ...
    pub fn apply_config_update(
        ...
        if let Some(thinking_str) = thinking {
            if !self.compat.supports_thinking() {
                ...
                match thinking_str.as_str() {
                    "enabled" => {
                        ...
                        self.thinking = Some(eggsplain_types::llm::ThinkingConfig::Enabled {
                            budget_tokens: budget,
                        });
                        changes.push(format!("thinking: enabled (budget: {budget})"));
                    }
                    "disabled" => {
                        self.thinking = Some(eggsplain_types::llm::ThinkingConfig::Disabled);
            ...
            && let Some(eggsplain_types::llm::ThinkingConfig::Enabled { budget_tokens }) =
                &mut self.thinking
        {
        ...
        if let Some(ref level_str) = compaction {
            match level_str.parse::<eggsplain_compact::CompactionLevel>() {
                Ok(new_level) => {
    ...
    async fn run_compaction(&mut self) -> Result<(), AgentError> {
        ...
        if should_compact {
            tracing::info!(target: "eggsplain_agent", last_input_tokens = self.compact_state.last_input_tokens, "context compaction triggered");
    ...
    pub async fn run_stop_hooks(&self) {
        if let Some(hook_engine) = &self.hooks {
            ...
            for msg in messages {
                tracing::info!(target: "eggsplain_agent", hook_message = %msg, "stop hook output");
            }
        }
    }
...
mod set_config_tests {
    ...
    use eggsplain_providers::{LlmProvider, ProviderError};
    use eggsplain_tools::registry::ToolRegistry;
    use eggsplain_types::llm::{LlmEvent, LlmRequest};
    ...
    fn make_engine(model: &str) -> super::AgentEngine {
        super::AgentEngine {
            ...
            thinking: None,
            compat: eggsplain_config::compat::ProviderCompat::anthropic_defaults(),
            ...
            allow_list: vec![],
            current_reasoning_effort: None,
            compact_config: eggsplain_config::compact::CompactConfig::default(),
            compact_state: super::CompactState::new(),
            plan_state: Default::default(),
            plan_active_flag: None,
            cache_detector: super::CacheBreakDetector::new(),
            compaction_level: eggsplain_compact::CompactionLevel::default(),
            toon_enabled: false,
            commands: crate::commands::default_registry(),
        }
    }
    ...
    fn set_config_changes_effort() {
        let mut engine =
            make_engine_with_compat("m", eggsplain_config::compat::ProviderCompat::openai_defaults());
    ...
    fn set_config_enables_thinking() {
        ...
        match &engine.thinking {
            Some(eggsplain_types::llm::ThinkingConfig::Enabled { budget_tokens }) => {
    ...
    fn set_config_disables_thinking() {
        let mut engine = make_engine("m");
        engine.thinking = Some(eggsplain_types::llm::ThinkingConfig::Enabled {
            budget_tokens: 8000,
        });
        let changes = engine.apply_config_update(None, Some("disabled".into()), None, None, None);
        match &engine.thinking {
            Some(eggsplain_types::llm::ThinkingConfig::Disabled) => {}
    ...
    fn set_config_all_fields_at_once() {
        let compat = eggsplain_config::compat::ProviderCompat {
    ...
    fn set_config_thinking_budget_ignored_when_disabled() {
        let mut engine = make_engine("m");
        engine.thinking = Some(eggsplain_types::llm::ThinkingConfig::Disabled);
        let changes = engine.apply_config_update(None, None, Some(20000), None, None);
        match &engine.thinking {
            Some(eggsplain_types::llm::ThinkingConfig::Disabled) => {}
...
mod phase6_tests {
    ...
    use eggsplain_tools::registry::ToolRegistry;
    use eggsplain_types::llm::{LlmEvent, LlmRequest};
    use eggsplain_types::skill_types::{ContextModifier, EffortLevel};
...
mod compact_tests {
    ...
    use eggsplain_tools::registry::ToolRegistry;
    use eggsplain_types::llm::{LlmEvent, LlmRequest};
    use eggsplain_types::message::{ContentBlock, Message, Role};
    use serde_json::json;
...
mod plan_mode_tests {
    ...
    use eggsplain_tools::registry::ToolRegistry;
    use eggsplain_types::llm::{LlmEvent, LlmRequest};
    use eggsplain_types::skill_types::{ContextModifier, PlanModeTransition};
