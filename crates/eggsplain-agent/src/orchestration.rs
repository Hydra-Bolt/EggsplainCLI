...
use eggsplain_protocol::events::{OutputType, ProtocolEvent, ToolCategory, ToolInfo, ToolStatus};
use eggsplain_protocol::writer::ProtocolEmitter;
use eggsplain_protocol::{ToolApprovalManager, ToolApprovalResult};
use eggsplain_types::message::ContentBlock;
use eggsplain_types::skill_types::ContextModifier;
use eggsplain_types::tool::ToolResult;
...
async fn execute_single(
    ...
    tracing::info!(target: "eggsplain_agent", tool = %name, call_id = %id, "tool execution started");
    ...
    let (result, modifier) = match registry.get(name) {
        Some(tool) => {
            ...
            let content = eggsplain_compact::compact_output(&content, compaction_level);
            let content = if toon_enabled {
                eggsplain_compact::compact_output_toon(&content)
            } else {
                content
            };
            (
    ...
    if let Some(hook_engine) = hooks {
        let messages = hook_engine
            ...
            .await;
        for msg in messages {
            tracing::info!(target: "eggsplain_agent", hook_message = %msg, "post-tool-use hook output");
        }
    }

    let duration_ms = start.elapsed().as_millis() as u64;
    tracing::info!(target: "eggsplain_agent", duration_ms, success = !result.is_error, "tool execution completed");
...
mod tests {
    ...
    impl Tool for MockDeferredTool {
        ...
        async fn execute(&self, input: serde_json::Value) -> eggsplain_types::tool::ToolResult {
            if input.get("tasks").is_none() {
                return eggsplain_types::tool::ToolResult {
                    content: "Missing or invalid 'tasks' array".to_string(),
                    is_error: true,
                };
            }
            eggsplain_types::tool::ToolResult {
                content: "ok".to_string(),
                is_error: false,
            }
        }
        fn category(&self) -> eggsplain_protocol::events::ToolCategory {
            eggsplain_protocol::events::ToolCategory::Exec
        }
    }
    ...
    async fn execute_single_deferred_tool_error_missing_required_appends_hint() {
        ...
        let (result, _) = execute_single(
            ...
            &call,
            None,
            eggsplain_compact::CompactionLevel::Off,
            false,
        )
        .await;
        if let ContentBlock::ToolResult {
