...
use eggsplain_providers::LlmProvider;
...
impl AgentBootstrap {
    ...
    pub async fn build(mut self) -> anyhow::Result<BootstrapResult> {
        ...
        tracing::info!(target: "eggsplain_agent", workspace = %cwd, "agent bootstrap: workspace cwd resolved");

        let provider = self
            .provider
            .unwrap_or_else(|| eggsplain_providers::create_provider(&self.config));

        let memory_dir = eggsplain_memory::paths::auto_memory_dir(cwd_path);

        let file_cache = if self.config.file_cache.enabled {
            Some(Arc::new(std::sync::RwLock::new(
                eggsplain_tools::file_cache::FileStateCache::new(&self.config.file_cache),
            )))
        } else {
            None
        };

        let mut registry = eggsplain_tools::registry::ToolRegistry::new();
        registry.register(Box::new(eggsplain_tools::read::ReadTool::new(
            file_cache.clone(),
        )));
        registry.register(Box::new(eggsplain_tools::write::WriteTool::new(
            file_cache.clone(),
        )));
        registry.register(Box::new(eggsplain_tools::edit::EditTool::new(file_cache)));
        registry.register(Box::new(eggsplain_tools::bash::BashTool::new(
            cwd_path.to_path_buf(),
        )));
        registry.register(Box::new(eggsplain_tools::grep::GrepTool::new(
            cwd_path.to_path_buf(),
        )));
        registry.register(Box::new(eggsplain_tools::glob::GlobTool::new(
            cwd_path.to_path_buf(),
        )));
        ...
        let mcp_manager = if !self.config.mcp.servers.is_empty() {
            match McpManager::connect_all(&self.config.mcp.servers).await {
                Ok(mgr) => {
                    let mgr = Arc::new(mgr);
                    eggsplain_mcp::tool_proxy::register_mcp_tools(
                        &mut registry,
                        &mgr,
                        &builtin_names,
        ...
        let skills = eggsplain_skills::loader::load_all_skills(
            cwd_path,
        ...
        let skill_checker = eggsplain_skills::permissions::SkillPermissionChecker::new(
        ...
        registry.register(Box::new(eggsplain_tools::tool_search::ToolSearchTool::new(
            tool_defs_snapshot,
        )));
