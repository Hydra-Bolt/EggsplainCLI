...
use eggsplain_skills::context_modifier::ContextModifier;
use eggsplain_skills::executor::{execute_fork, prepare_inline_content};
use eggsplain_skills::hooks::{parse_skill_hooks, to_hook_defs};
use eggsplain_skills::permissions::{SkillPermission, SkillPermissionChecker};
use eggsplain_skills::types::{ExecutionContext, SkillMetadata};
use eggsplain_types::tool::{JsonSchema, ToolResult};
...
pub struct SkillTool {
    ...
    /// Session ID passed to prepare_inline_content for ${EGGSPLAIN_SESSION_ID} substitution.
...
impl Tool for SkillTool {
    ...
    fn context_modifier_for(&self, input: &serde_json::Value) -> Option<ContextModifier> {
        ...
        eggsplain_skills::context_modifier::from_skill(skill)
    }
...
mod tests {
    use super::*;
    use eggsplain_skills::permissions::SkillPermissionChecker;
    use eggsplain_skills::types::{ExecutionContext, LoadedFrom, SkillSource};
    use serde_json::json;
...
mod supplemental_tests {
    ...
    use eggsplain_skills::permissions::SkillPermissionChecker;
    use eggsplain_skills::types::{ExecutionContext, LoadedFrom, SkillMetadata, SkillSource};
    ...
    async fn tc_13_8_full_variable_substitution_integration() {
        let mut skill = make_skill("my-skill", "Run ${EGGSPLAIN_SKILL_DIR}/tool.sh $ARGUMENTS[0]");
...
mod phase7_tests {
    ...
    use eggsplain_tools::Tool;
    use eggsplain_types::message::TokenUsage;
