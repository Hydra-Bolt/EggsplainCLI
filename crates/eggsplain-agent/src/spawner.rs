...
use eggsplain_tools::bash::BashTool;
use eggsplain_tools::edit::EditTool;
use eggsplain_tools::glob::GlobTool;
use eggsplain_tools::grep::GrepTool;
use eggsplain_tools::read::ReadTool;
use eggsplain_tools::registry::ToolRegistry;
use eggsplain_tools::write::WriteTool;
...
// Re-export from eggsplain-types — single source of truth
pub use eggsplain_types::spawner::{ForkOverrides, Spawner, SubAgentConfig, SubAgentResult};
...
impl AgentSpawner {
    ...
    pub async fn spawn_one(&self, sub_config: SubAgentConfig) -> SubAgentResult {
        ...
        tracing::info!(target: "eggsplain_agent", cwd = %self.cwd.display(), "sub-agent spawned with workspace cwd");
...
fn build_tool_registry(allowed: &[String], cwd: &Path) -> ToolRegistry {
    let all_tools: Vec<(&str, Box<dyn eggsplain_tools::Tool>)> = vec![
